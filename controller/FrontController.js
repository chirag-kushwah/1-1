...
cloudinary.config({
  cloud_name: "djro5s4md",
  api_key: "258818619823855",
  api_secret: "rjNTUs6ZKHJoVRUeG30nK8PQUFY",
});
...
class FrontController {
  ...
  static studentInsert = async (req, res) => {
    try {
      ...
      if(data) {
        const token = jwt.sign({ ID: data._id }, "chirag@123");
        // console.log(token)
  ...
  static verifyLogin = async (req, res) => {
    try {
      ...
      if (user != null) {
        ...
        if (isMatched) {
          // if(user.role=="admin"){
          //   const token = jwt.sign({ ID: user._id }, "chirag@123");
          //   // console.log(token)
          //   res.cookie("token", token);
